...
Citizen.CreateThread(function()
	...
	while true do 
		Wait(0);
		local enabled = Config.tag_hud.enabled;
		if enabled then 
			local disp = Config.tag_hud.defaultText;
			local scale = Config.tag_hud.fontSize;
			local x = Config.tag_hud.x;
			local y = Config.tag_hud.y;
