...
local function CreateTagMenu()
    local menuTitle = Config.playerNameTitle and playerName or Config.TagMenuTitle
    tagsMenu = NativeUI.CreateMenu(menuTitle, "Select a Tag", Config.MenuPos.x, Config.MenuPos.y)
    if Config.useTagMenuImage then
        local RuntimeTXD = CreateRuntimeTxd('Custom_Menu_Head3')
        local Object = CreateDui(Config.TagMenuImage, 512, 128)
